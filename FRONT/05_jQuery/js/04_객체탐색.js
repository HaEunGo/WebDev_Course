$(document).ready(() => {
    // 1. Filtering 메소드
    //  - Filtering 방법으로는 선택자와 함수를 제공한다.(요소.선택자)
    // 
    //  * 필터링 관련 선택자
    ...
    //    선택자:not(인) - 인자 값과 일치하지 않는 요소를 선택
    ...
    }).css({backgroundColor: 'brown', color: 'white'});

    // 선택한 요소들 중 인덱스에 해당하는 요소를 가져와서 텍스트를 변경
    $('h4').eq(4).text('eq()에 의해서 선택되었습니다.');
    // $('h4:eq(4)').text('eq()에 의해서 선택되었습니다.');

    // 인자 값과 일치하지 않는 요소를 가져와서 스타일 변경
    // $('h4').not('.test').css({backgroundColor: 'brown', color: 'white'});
    // $('h4:not(".test")').css({backgroundColor: 'brown', color: 'white'});
    $('h4:not(.test)').css({backgroundColor: 'brown', color: 'white'});

    $('h4').filter('.test').text('aaaaaaaaaaa');
    ...
    // $('span').parentsUntill('div').css('background-color', 'lightgreen');

    // 3. Descendants 메소드
    console.log($('.wrap').childeren());
    console.log($('.wrap').find('span'));

    // 클래스명이 wrap인 태그의 자손의 스타일 변경
    $('.wrap').childeren().css({color: 'red', border: '2px solid'});

    // 클래스명이 wrap인 태그의 자손의 자손들의 스타일 변경
    $('.wrap').childeren().childeren().css({color: 'blue', border: '2px solid'});

    // 클래스명이 wrap인 태그의 자손의 자손들 중 ul 태그의 스타일 변경
    $('.wrap').childeren().childeren('ul').css({color: 'yellow', border: '2px solid'});
    
    // 클래스명이 wrap인 태그의 자손의 자손의 자손들 중 li 태그의 스타일 변경
    $('.wrap').childeren().childeren().childeren('li').css({color: 'green', border: '2px solid'});

    // 클래스명이 wrap인 태그의 모든 후손 중 span 태그의 스타일 변경
    $('.wrap').find('span').css({color: 'red', border: '2px solid'});
