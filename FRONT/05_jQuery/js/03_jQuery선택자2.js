$(document).ready(() => {
    ...
    //  2) 후손 선택자 : 부모 후손
    $('div h3').css('backbround-color', 'skyblue');
    $('div .test').css('backbround-color', 'tomato');

    // 2. 기본 속성 선택자
    // $('input[class]').css('backbround-color', 'tomato');
    ...
    $('input:checkbox').change((event) => {
        ...
        if(target.prop('checked')) {
            target.css({width: '50px', heigtht: '50px'});
        } else {
            target.css({width: '15px', heigtht: '15px'});
        }
    });
    ...
    // 6. input 상태에 대한 선택자
    // $('input:enabled').css('background-color', 'salmon');
    $('input:disabled').css('background-color', 'salmon');
    ...
    // prop : 값의 존재 유무를 논리값(true, false)으로 출력
