...
<body>
	<c:set var="path" value="${ pageContext.request.contextPath }" /> <!-- path로 변수 담아줌 -->
	<h2>AJAX</h2>
	...
	<script>
		function jsAjaxTest1() {
				...
				/* 브라우저 버전별로 ajax 지원 여부 및 호환성 검사를 해야 한다.
				...
				*/
				...
				xhr.onreadystatechange = () => {
					...
					*/
					...
					if (xhr.readyState === 4) {
						/*
							2) statue : HTTP 응답 상태 코드 (응답이 정상적으로 이루어졌는지 확인)
								- 200 : OK
								- 404 : Not Found
								...
				...
				xhr.open("GET", "${path}/jsAjax.do?name=홍길동&age=20", true);
				
				// 4. send() : 서버에 데이터를 전송(요청)한다.
				xhr.send();
				/* 서블릿으로 데이터를 요청하는 것이 없기 때문에 404 에러가 뜬다. */
		}
	</script>
	
	<h4> 2) POST 방식으로 서버 데이터 전송 및 응답</h4>
	...
	<script>
		function jsAjaxTest2() {
			...
			//POST 방식으로 보낼 때 추가로 필요함 (MIME타입 : Content-type - 응답해주는 데이터 타입)
			/* application : 이진타입 / 헤더 변경해서 넘어가야 한다. */
	...
	$(document).ready(() => {
		$("#btn1").on("click", () => {
			...
			$.ajax({
				...
				url : "jqAjax1.do", // 데이터를 전송(요청)할 URL(필수)
				date : { // 요청 시 전달할 파라미터 설정 왼:속성명 오:변수명
					input: input // 이름 동일할 시 input 하나만 적어줘도 된다.
				},
				success: function(result) {
					// AJAX 통신 성공 시 처리할 콜백 함수 지정
					// 매개변수(result)는 서버에서 응답이 왔을 때 그 값이 저장되는 변수 (임의로 변수명 지정 가능)
					console.log(result);
				...
				complete: function() {
					// 통신의 성공 여부와 상관없이 실행되는 콜백 함수 지정 (보통 success, error를 많이 사용함)
		...
		$("#btn3").on("click", () => {
			let userNo = $("#userNo").val();
				...
				$.ajax({
					type: "get",
					url: "jqAjax2.do",
					dataType: "json", // 응답 데이터 형식 (미작성 시 자동으로 응답 데이터 형식을 판단하여 지정한다. 대소문자 상관X)
					data: {
						userNo
					},
					success: function(obj) {
						...
						if(obj !== null) {
							// obj가 null이 아니라면 아래와 같은 결과를 textarea1에 결과값으로 출력해준다.
							result = "no : " + obj.no + ", name : " + obj.name + ", age: " + obj.age + ", gender: "+ obj.gender;
						} else {
							// 그 반대라면(null이라면) 아래와 같은 결과를 textarea1에 결과값으로 출력해준다.
							result = "사용자 정보가 없습니다.";
						}
	...
	<script>
		$(document).ready(() => {
			// btn4 이벤트 등록, 콜백 함수
			$("#btn4").on("click", () => {
				// 두 개 중에 선택한 값으로 가져오기 (input태그중에 속성이 name에 gender가 들어가 있으면 선택하면서 그 중에서도 checked가 되어있는 요소를 선택한 뒤 그 값의 val()을 가져오기)
				...
				$.ajax({
					...
					// 서버에서 전달해주는 데이터타입
					dataType: 'json',
					// 데이터 값, 넘길 파라미터가 여러개라면 콤마(,)를 활용해 적어주면 된다.
					data: {
						gender
					},
					// 서버에서 응답이 성공적으로 왔을 때의 결과 (list로 받아오기)
					success: (list) => {
						let result = "";
						// jQuery에 있는 each 메소드를 활용하여 값 가져오기
						$.each(list, (i) => {
							result +=
