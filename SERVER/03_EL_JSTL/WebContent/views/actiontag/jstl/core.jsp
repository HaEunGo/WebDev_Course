...
<body>
	...
	<%-- pageContext.setAttribute("num1", "10"); 액션 태그로 만들어주는 것과 동일하다. --%>
	<c:set var="num1" value="30" />
	
	<!-- 자바 코드로 봤을 때 pageContext.setAttribute("num2", "20"); 와 동일하다.-->
	<c:set var="num2" value="20" scope="request" />
	<!-- EL식을 사용해서 미리 만들어진 변수를 사용할 수 있다. session 스코프에 저장됨 -->
	...
	<!-- 표현식, 스크립트릿 요소에서는 해당하는 영역 객체에서 꺼내와야 한다. -->
	...
	<p>
		지정한 변수를 모든 scope에서 검색해서 삭제한다.(scope 속성을 지정해서 특정 scope의 변수만 삭제 가)
	</p>
	
	<!-- 영역 객체 scope의 이름이 다르기 때문에 var 변수 이름이 같아도 괜찮다 -->
	...
	<!-- scope 지정하지 않고 지우면 모든 영역에 있는 result라는 속성명의 값을 다 지운다. -->
	...
	<!-- step : begin의 수보다 step 수만큼 증가를 시킬 수 있다. -->
	...
	<%
		...
		list.add(new Person("돌멩이", 28, '남'));
		list.add(new Person("짱구", 10, '남'));
		list.add(new Person("초롱이", 30, '여'));
	...
	<table border="1">
		...
		<!-- status : 현재 반복에 대한 상태를 가지고 있는 변수 -->
		<c:forEach var="person" items="${ list }" varStatus="status">
			<tr>
				<td>${ status.index }</td> <!-- 인덱스는 0부터 -->
	...
	<p>
		문자열에 포함된 구분자를 통해 토큰을 분리해서 반복 처리하는 태그이다.
