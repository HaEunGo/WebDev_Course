...
public class MethodServlet extends HttpServlet {
    ...
    @Override
//    request : 사용자 요청에 대한 정보
//    response : 사용자에게 응답해주는 용도 (요청하게 될 사용자의 정보도 있음)
...
//    	파라미터 사용 시, 동일한 이름의 속성으로 여러 개가 전달이 되어도 가장 먼저 받는 값만 출력해준다.
//    	String food = request.getParameter("food");
//    	String food = request.getParameter("food2");
    	
//    	해당 name 속성을 가지는 요소들!!의 value 값들을 문자열의 배열로 읽어온다.
...
//    	응답 화면에 대한 설정 (문서 타입, 인코딩 정보)
//    	응답 화면이 문서형태의 html이고, 문자 셋은 UTF-8이다.
//    	아래의 과정이 없으면 인코딩 문제로 한글이 깨진다.
    	response.setContentType("text/html;charset=UTF-8");
    	
//    	사용자에게 응답 화면을 출력하기 위한 스트림 생성 (문자형 기반 출력스트림)
...
//    	자바 코드로 응답 화면을 작성하고 있는 과정
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	// POST 방식으로 넘겨받은 데이터가 영어 또는 숫자가 아닌 경우 아래와 같이 추가적으로 인코딩 처리가 필요하다.
