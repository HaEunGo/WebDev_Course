...
-- EMPLOYEE 테이블에서 직급 코드(JOB_CODE)가 J2이거나 급여가 400만원 이상인 직원들의 모든 컬럼 조회
SELECT *        -- 모든 컬럼 조회
FROM EMPLOYEE
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY >= 3500000 AND SALARY <= 6000000;  -- 조건을 따로 작성해서 연산자로 이어준다.
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY BETWEEN 3500000 AND 6000000;   -- 하한값 : 350만원 / 상한값 : 600만원
...
-- EMPLOYEE 테이블에서 입사일이 '90/01/01' ~ '01/01/01'인 사원의 모든 컬럼 조회 (문자 데이터 형태)
SELECT *
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 김씨 성이 아닌 직원의 사번, 사원명, 입사일 조회 (NOT LIKE)
SELECT EMP_ID, EMP_NAME, HIRE_DATE
FROM EMPLOYEE
