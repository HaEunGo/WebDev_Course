    ...
    <TCL(Transaction Control Language)>
        ...
        * 트랜잭션
            ...
            ATM에서 현금 출금
                ...
                5. 실제 현금 인출
                6. 완료
            - 각각의 업들을 묶어서 논리적인 하나의 작업 단위로 만들어 버리는 것을 트랜잭션이라고 한다.
            - 데이터의 변경 사항(DML)들을 묶어서 하나의 트랜잭션에 담아서 처리한다.
                (COMMIT 하기 전까지의 변경사항들을 하나의 트랜잭션에 담게 된다.)
            - COMMIT(트랜잭선 종료 처리 후 저장), ROLLBACK(트랜잭션 취소), SAVEPOINT(임시저장)을 통해서 트랜잭션을 제어한다.
              COMMIT
              ...
              SAVEPOINT
                저장점을 정의해두면 ROLLBACK을 진행할 때 전체 작업을 ROLLBACK 하는 것이 아닌 SAVEPOINT까지의 일부(저장점까지)만 ROLLBACK 시킬 수 있다.
...
-- CREATE TABLE EMP_01 생성하면서 자동으로 COMMIT이 되므로 그 시점으로 돌아간다.
ROLLBACK;
