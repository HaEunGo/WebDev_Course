    ...
    <문자 관련 함수>
        1) LENGTH / LENGTHB
         ...
         - LENGTHB(컬럼 | '문자값') : 글자의 바이트 수 반환
            ...
            * DUAL 테이블
             - sys 사용자(관리자 계정)가 소유하는 테이블
             - sys 사용자가 소유하지만 모든 사용자에서 접근이 가능하다.
...
/*
        2)INSTR
            - INSTR(컬럼|'문자값', '찾을 문자값'[, POSITON[, OCCURRENCE]])
            - 지정한 위치부터 지정된 숫자 번째로 나타나는 문자의 시작 위치를 반환한다.
*/

SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL;    -- 3번째 자리 B위치 반환
SELECT INSTR('AABAACAABBAA', 'B', 1) FROM DUAL; -- 3번째 자리 B위치 반환, 포지션 값 지정 안해줬을때 1이 기본값
SELECT INSTR('AABAACAABBAA', 'B', 1, 2) FROM DUAL; -- 찾고 싶은 문자값의 위치값, 두 번째 B 위치 반환
...
/*
    3) LPAD / RPAD
        - LPAD / RPAD(컬럼|'문자값', 최종적으로 반환할 문자의 길이(바이트 단위 지정)[, 덧붙이고자 하는 문자])
...
-- SELECT LTRIM('123123KH', '321') FROM DUAL; -- 첫문자를 공백으로 넣게 되면 적용X, 문자열의 순서를 바꿔도 동일하게 제거된다.
...
/*
    6) SUBSTR
        ...
        - 문자열에서 지정한 위치부터 지정한 개수(LENGTH)만큼의 문자열을 추출해서 반환한다.

*/

SELECT SUBSTR('SHOWMETHEMONEY', 7) FROM DUAL;   -- 지정된 포지션부터 끝까지 출력
SELECT SUBSTR('SHOWMETHEMONEY', 5, 2) FROM DUAL;    -- 지정된 포지션, 원하는 길이 지정
SELECT SUBSTR('SHOWMETHEMONEY', 1, 6) FROM DUAL;
SELECT SUBSTR('SHOWMETHEMONEY', -8, 3) FROM DUAL;   -- 음수는 오른쪽 부터 위치 선정 후 출력 : THE
...
-- EMPLOYEE 테이블에서 여자 사원만 (사원명, 성별코드)
SELECT EMP_NAME AS "사원명", SUBSTR(EMP_NO, 8, 1) AS "성별코드"
FROM EMPLOYEE
WHERE SUBSTR (EMP_NO, 8, 1) = '2'; -- = 2;로 적어도 된다.
...
SELECT CONCAT('가나다라', 'ABCD') FROM DUAL;    -- 무조건 두 개의 문자정보만 받아서 하나의 문자데이터로 출력할 수 있다.
SELECT '가나다라' || 'ABCD' FROM DUAL;          -- 차이점은 여러 개의 문자열을 연결할 수 있다.
...
-- SELECT 10 % 3 FROM DUAL;    -- ERROR 발생 (연산자 사용 불가)
...
-- EMPLOYEE 테이블에서 직원명, 입사일, 근무개월 수
SELECT EMP_NAME, HIRE_DATE, FLOOR(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)) AS "근무개월 수"
FROM EMPLOYEE;
...
/*
        4) NEXT_DAY
            ...
            - 결과값은 DATE이다.
*/
SELECT SYSDATE, NEXT_DAY('21/09/27', '금요일') FROM DUAL;  -- 특정 날짜는 문자형태로 입력해주면 된다.
SELECT SYSDATE, NEXT_DAY(SYSDATE, '금요일') FROM DUAL;
SELECT SYSDATE, NEXT_DAY(SYSDATE, '금') FROM DUAL;
SELECT SYSDATE, NEXT_DAY(SYSDATE, 4) FROM DUAL;   -- 1 : 일요일, 2 : 월요일, ..., 7 : 토요일 / 넘어가는 수를 적으면 에러발생
SELECT SYSDATE, NEXT_DAY(SYSDATE, 'SUNDAY') FROM DUAL; -- ERROR : 현재 언어가 KOREAN이기 때문에
...
SELECT SYSDATE, NEXT_DAY(SYSDATE, '금') FROM DUAL;   -- ERROR : 현재 언어 AMERICAN
...
SELECT LAST_DAY('20210810') FROM DUAL;  -- 문자 리터럴로 만들어도 자동으로 형변환된다.
...
-- ORDER BY 3, 4, 5;   -- 추출한값으로 정렬 (연도기준 오름차순, 중복시 월기준 오름차순, 또 중복시 일기준 오름차순)
-- ORDER BY HIRE_DATE;    -- 입사연월일 기준으로 정렬
...
SELECT TO_CHAR(1234, '$999999') FROM DUAL;  -- 임의의 화폐로도 지정할 수 있다.
SELECT TO_CHAR(1234, 'L') FROM DUAL;        -- 뒤에 숫자 범위 지정해주지 않으면 #으로 표시된다.
...
-- YY는 무조건 현재 세기를 반영하고, RR는 50미만이면 현재 세기를 반영, 50이상이면 이전 세기를 반영한다.
...
-- 일에 대한 포맷
SELECT TO_CHAR(SYSDATE, 'D'),   -- 1주를 기준으로 몇일 째인지 표시
        TO_CHAR(SYSDATE, 'DD'), -- 1달을 기준으로 몇일 째인지 표시
        TO_CHAR(SYSDATE, 'DDD') -- 1년을 기준으로 몇일 째인지 표시
FROM DUAL;
...
SELECT EMP_NAME,
        TO_CHAR(HIRE_DATE, 'YYYY"년" MM"월" DD"일" (DY)') AS "입사일"    -- 년, 월, 일 문자는 포맷 문자(yy, mm, dd)가 아니기 때문에 오류가 나올 수 있으므로, 큰 따옴표로 묶어줘야한다.
FROM EMPLOYEE;
...
SELECT TO_DATE (20210928111530) FROM DUAL;      -- 날짜 포맷 변경 후 출력
...
-- YY와 RR 비교
SELECT TO_DATE ('210928', 'YYMMDD') FROM DUAL;  -- YY : 무조건 현재 세기(21세기)
SELECT TO_DATE ('980928', 'YYMMDD') FROM DUAL;

SELECT TO_DATE ('210928', 'RRMMDD') FROM DUAL;  -- RR : 해당 값이 50 이상이면 이전 세기, 50 미만이면 현재 세기
...
SELECT '123' + '456' FROM DUAL;             -- 자동으로 숫자 타입으로 형변환 후 연산처리를 해준다.
SELECT '123' + '456A' FROM DUAL;            -- 에러 발생(숫자 형태의 문자들만 자동형변환 된다.)
SELECT '10,000,000' + '500,000' FROM DUAL;  -- 에러 발생(콤마도 문자다.)
SELECT TO_NUMBER('10,000,000', '99,999,999') + TO_NUMBER('500,000', '999,999') FROM DUAL;    -- 숫자값을 형변환 해주면서 포맷 설정까지 해주면 연산이 가능.
...
/*
    <NULL 처리 함수>
        ...
        2) NVL2
            ...
            - 컬럼 값이 NULL이 아니면 바꿀값1, 컬럼값이 NULL이면 바꿀값 2로 변경하여 반환한다.
...
/*
    <선택함수>
        ...
        1) DECODE
            [표현법]
               DECODE(컬럼|계산식, 조건값 1, 결과값 1, 조건값2, 결과값 2, ..., 결과값)
...
SELECT EMP_ID,
        EMP_NAME,
        EMP_NO,
--      SUBSTR(EMP_NO, 8, 1)    -> 8번째 자리 1개의 값 가져오기
        DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남자', '2', '여자', '잘못된 주민번호입니다') AS "성별"
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 사원명, 직급 코드, 기존 급여, 인상된 급여를 조회
-- 직급코드가 J7인 사원은 급여를 10% 인상 (* 1.1)
-- 직급코드가 J6인 사원은 급여를 15% 인상 (* 1.15)
-- 직급코드가 J5인 사원은 급여를 20% 인상 (* 1.2)
-- 그 외의 직급의 사원은 급여를 5%만 인상 (* 1.5)

SELECT EMP_NAME AS "사원명", 
        JOB_CODE AS "직급코드",
        SALARY AS "기존급여",
        DECODE(JOB_CODE, 'J7', SALARY * 1.1, 'J6', SALARY * 1.15, 'J5', SALARY * 1.2, SALARY * 1.05) AS "인상 급여"
FROM EMPLOYEE;
...
/*
        4) COUNT
            ...
            - COUNT(*) : 조회 결과에 해당하는 몸든 행의 개수를 반환한다.
            - COUNT(컬럼명) : 제시한 컬럼 값이 NULL이 아닌 행의 개수를 반환한다.
            - COUNT(DISTINCT) : 해당 컬럼 값의 중복을 제거한 행의 개수를 반환한다.
*/
...
-- 퇴사한 직원의 수(ENT_DATE가 NULL인 경우 개수를 세지 않는다.)
