...
/*
    ...
    <서브 쿼리 구분>
        ...
        1. 단일행 서브 쿼리        : 서브 쿼리의 조회 결과 값의 개수가 오로지 1개 일 때
...
WHERE SALARY < (
    SELECT AVG(SALARY)
    FROM EMPLOYEE
) ORDER BY SALARY;   -- EMP_NAME, JOB_CODE 내가 원하는 기준으로 정렬


-- 2) 최저 급여를 받는 직원의 사번, 이름, 직급 코드, 급여, 입사일 조회
-- 최저 급여 조회
SELECT MIN(SALARY)
FROM EMPLOYEE; --  결과 값 1행 1열 (1380000)
...
-- 3) 노옹철 사원의 급여보다 더 많이 받는 사원의 사번, 사원명, 부서명, 직급 코드, 겁여 조회
-- 노옹철 사원의 급여 조회
SELECT SALARY
FROM EMPLOYEE
WHERE EMP_NAME = '노옹철';

-- 위 쿼리를 서브 쿼리를 사용하는 메인 쿼리 작성(ANSI)
...
-- * 서브 쿼리는 WHERE 절 뿐만 아니라, SELECT / FROM / HAVING 에서 사용이 가능하다.
...
-- 5) 전지연 사원이 속해있는 부서명을 조회(단, 전지연 사원은 제외)
--      시번, 사원명, 전화번호, 직급명, 부서명, 입사일
-- 전지연 사원이 속해있는 부서 조회
SELECT DEPT_CODE
FROM EMPLOYEE
WHERE EMP_NAME = '전지연';
...
-- JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID); -- 1. 모든 테이블 JOIN하여 하나의 행으로 출력

-- SELECT E.EMP_ID, E.EMP_NAME, E.PHONE, J.JOB_NAME, D.DEPT_TITLE, E.HIRE_DATE
-- FROM EMPLOYEE E
-- JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
-- JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID); -- 2. 원하는 데이터 SELECT
...
WHERE E.DEPT_CODE = (
    ...
    FROM EMPLOYEE
    WHERE EMP_NAME = '전지연'
) AND E. EMP_NAME != '전지연';     -- 3. 원하는 데이터 출력을 위한 서브쿼리문 작성
...
/*
    <다중행 서브 쿼리>
        서브 쿼리의 조회 결과 값의 행의 개수가 여러 행일 때
        ...
        ANY : 여러 개의 값들 중에서 한 개라도 만족하면 TRUE, IN과 다른 점은 비교 연산자를 사용한다는 점이다.
            ...
            SALARY < ANY(...) : 최대값 보다 크면 TRUE 리턴
        ALL : 여러 개의 값을 모두와 비교하여 만족해야만 TRUE
            SALARY > ANY(...) : 최대값 보다 크면 TRUE 리턴
            SALARY < ANY(...) : 최소값 보다 작으면 TRUE 리턴
*/
...
FROM EMPLOYEE
WHERE MANAGER_ID IS NOT NULL; -- (204, 100, 200, 211, 207, 214)

-- 사번이 위와 같은 직원들의 사번, 이름, 부서 코드, 구분(사수) 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, '사수' AS "구분"
FROM EMPLOYEE
WHERE EMP_ID IN(204, 100, 200, 211, 207, 214);
...
-- 4) 과장 직급임에도 차장 직급의 최대 급여보다 많이 받는 직원의 사번, 이름, 직급, 급여 조회 (사원 -> 대리 -> 과장 -> 차장 -> 부장)
-- 차장 직급들의 급여 조회
SELECT SALARY
FROM EMPLOYEE E
JOIN JOB J ON (E.JOB_CODE = J.JOB_CODE)
...
SELECT E.EMP_ID, E.EMP_NAME, J.JOB_NAME, E.SALARY
FROM EMPLOYEE E
JOIN JOB J USING (JOB_CODE)    -- 동일한 컬럼명을 적어줘야 한다.
...
-- 1) 하이유 사원가 같은 부서 코드, 같은 직급 코드에 해당하는 사원들 조회
...
WHERE (DEPT_CODE, JOB_CODE) = (
    ...
    FROM EMPLOYEE
    WHERE EMP_NAME = '하이유'
) /*AND*/;  -- 앞의 조건 AND 뒤의 조건에 부합하는 하나의 결과물로 출력
...
-- 박나라 사원의 직급 코드와 사수의 사번을 조회 --> 결과 값 : 1행의 여러열
...
-- 다중행 다중열 서브 쿼리를 사용해서 각 직급별로 최소 급여를 받는 사원의 사번, 이름, 부서 코드, 급여 조회
...
/*
    <인라인 뷰>
        FROM 절에 서브 쿼리를 제시하고, 서브 쿼리를 수정한 결과를 테이블 대신에 사용한다.
*/
...
-- FROM -> SELECT(순번 정해짐) -> ORDER BY(정렬)

-- 해결 방법
-- ORDER BY 한 결과값을 가지고 ROWNUM을 부여하면 된다. (인라인 뷰 사용)

SELECT ROWNUM AS "VUDRB",
        EMP_NAME AS "사원명",
        SALARY AS "급여"
FROM (
...
/*
    <RANK 함수>
        ...
        RANK() OVER(정렬 기준) : 동일한 순위 이후의 등수를 동일한 인원 수만큼 건너뛰고 순위를 계산한다. (ex. 공동 1위가 2명이면 그 다음 순위는 3위)
