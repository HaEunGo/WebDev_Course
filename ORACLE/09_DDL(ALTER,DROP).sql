...
-- ALTER TABLE DEPT_COPY MODIFY NUM NUMBER; -- ERROR
...
--      데이터 값이 기록되어 있어도 같이 삭제된다.(단, 삭제된 컬럼 복구(ROLLBACK)는 불가능)
...
-- DEPT_COPY 테이블에
-- DEPT_ID는 PK 제약조건 추가 (PRIMARY KEY)
...
-- EMPLOYEE 테이블 DEPT_CODE, JOB_CODE FOREIGN KEY 제약 조건을 적용
...
-- SYS_C007107 --> DEPT_COPY_NAME_NN --> 제약조건 확인하는 구문 실행하면 컴퓨터에서 자동으로 생성해준다. (그때그때마다 변경됨)
-- ALTER TABLE DEPT_COPY MODIFY DEPT_NAME NOT NULL;    -- 제약조건 추가
ALTER TABLE DEPT_COPY RENAME CONSTRAINT SYS_C007338 TO DEPT_COPY_DEPT_NAME_NN;

--  3-3) 테이블명 변경
--      1) ALTER TABLE 테이블명 RENAME TO 변경할테이블명
--      2) RENAME 기존테이블명 TO 변경할테이블명
-- DEPT_COPY --> DEPT_TEST로 변경
ALTER TABLE DEPT_COPY RENAME TO DEPT_TEST;  -- ALTER TABLE은 테이블을 수정하는 구문(테이블객체)
RENAME DEPT_COPY TO DEPT_TEST;  -- 객체이름 변경
-- 결과는 동일함
...
-- 바로 MEMBER_GRADE를 실행시킬 경우 에러발생 : 부모테이블에서 자식테이블을 참조하기 때문에 참조무결성 유지X
DROP TABLE MEMBER_GRADE;

-- 만약 삭제하고자 한다면
-- 1) 자식 테이블 먼저 삭제한 후 부모테이블을 삭제하는 방법
DROP TABLE MEMBER;  -- 멤버 테이블 삭제 먼저 진행
DROP TABLE MEMBER_GRADE;   

-- 2) 부모 테이블을 삭제할 때 제약조건도 함께 삭제하는 방법 (CASCADE CONSTRAINT) -> 제약조건, 테이블을 다같이 삭제시켜준다.
DROP TABLE MEMBER_GRADE CASCADE CONSTRAINT;


-- DROP 실습을 위한 부모테이블, 자식테이블 생성
