...
--SELECT DEPT_CODE, COUNT(*)  -- 에러가 난다. 카운트는 한 개로, DEPT_CODE는 여러 개가 있기 때문
SELECT DEPT_CODE  -- 그룹별로 카운팅 된 수를 출력해준다.
FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE NULLS FIRST; -- 정렬
...
-- 2. 급여의 평균(AVG) / 3. NVL사용해서 NULL값 0으로 변환 / 4. FLOOR 반내림 / 5. TO_CHAR 숫자 값 (,)정리
FROM EMPLOYEE
GROUP BY JOB_CODE  -- 1. 직급 그룹화
ORDER BY JOB_CODE;

-- 부서별 사원 수, 보너스를 받는 사원 수, 급여의 합, 평균 급여, 최고 급여, 최저 급여를 부서별 내림차순으로 조회
...
FROM EMPLOYEE
GROUP BY DEPT_CODE   -- 무조건 컬럼명으로 작성해야 한다. (순번, 별칭X)
ORDER BY DEPT_CODE DESC NULLS LAST;

-- 성별 별 사원 수
SELECT SUBSTR(EMP_NO, 8, 1) AS "성별"
FROM EMPLOYEE
GROUP BY SUBSTR(EMP_NO, 8, 1)   -- 컬럼, 계산식, 함수 호출 구문이 올 수 있다.(단, 컬럼 순번, 별칭은 사용할 수 X)
ORDER BY "성별";
...
/*
    ...
    * 실행 순서
        ...
        FROM        -- 1 : 조회하고자 하는 테이블링
        WHERE       -- 2 : 조건식
        GROUP BY    -- 3 : 그룹 기준에 해당하는 컬럼명 | 계산식 | 함수식
        HAVING      -- 4 : 그룹에 대한 조건식
        ORDER BY    -- 6 : 컬럼명 | 별칭 | 컬럼 순번 [ASC | DESC][NULLS FIRST | LAST]
*/
...
FROM EMPLOYEE
-- WHERE FLOOR(AVG(NVL(SALARY, 0))) >= 3000000 -- 에러 발생 / 순서
GROUP BY DEPT_CODE
...
-- CUBE(컬럼1, 컬럼2, ...) -> 컬럼 1을 가지고 중간 집계를 내고, 컬럼 2를 가지고도 중간 집계를 낸다. 즉, 전달되는 모든 컬럼을 집계한다.
SELECT DEPT_CODE, JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
...
/*
    <집합 연산자>
        여러 개의 쿼리 문을 가지고 하나의 쿼리문으로 만드는 연산자이다.
        
        UNION : 두 쿼리문을 수행한 결과값을 더한 후 중복되는 행은 제거한다. (합집합)
        UNION ALL : UNION과 동일하게 두 쿼리문을 수행한 결과 값을 더하고 중복은 허용한다. (합집합)
        INTERSECT : 두 쿼리문을 수행한 결과값에 중복된 결과값만 추출한다. (교집합)
        MINUS : 선행 결과값에서 수행 결과값을 뺀 나머지 결과값만 추출한다. (차집합)
*/

-- EMPLOYEE 테이블에서 부서 코드가 D6인 사람들만 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'; -- 6명 조회

-- EMPLOYEE 테이블에서 급여가 300만원 초과인 사람들만 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE SALARY > 3000000; -- 8명 조회 -> 중복되는 행은 자동으로 제거되고, 각각의 컬럼이 동일해야 한다.
...
/*
    <GROUPING SET>
        그룹 별로 처리된 여러 개의 SELECT문을 하나로 합친 결과를 원할 때 사용한다.
*/
