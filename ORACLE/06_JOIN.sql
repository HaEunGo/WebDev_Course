/*
    <JOIN>
        한 개 이상의 테이블에서 데이터를 조회하고자 할 때 사용되는 구문이다.
        무작정 데이터를 가져오는 게 아니라 각 테이블 간에 공통된 컬럼(데이터 타입)으로 데이터를 합쳐서 하나의 결과(RESULT SET)으로 조회한다.
        
        1. 등가 조인 (EQUAL JOIN) / 내부 조인(INNER JOIN)
            연결시키는 컬럼의 값이 일치하는 행들만 조인되어 조회된다. (일치하는 값이 없는 행은 조회X)
            
            1) 오라클 전용 구문
                [표현법]
                    SELECT 컬럼, 컬럼, 컬럼, ...
                    FROM 테이블1(기존), 테이블2 (변경될)
            ...
            2) ANSI 표준 구문
                [표현법]
                    SELECT 컬럼, 컬럼, 컬럼, ...
                    FROM 테이블 1
                    JOIN 테이블 2 ON/USING(테이블1, 컬럼명 = 테이블 2, 컬럼명);
...
-- 각 사원들의 사번, 사원명, 부서 코드, 부서명을 조회(각각 조회)
...
-- 각 사원들의 사번, 사원명, 부서 코드, 직급명을 조회 (각각 조회)
...
-- 오라클 구문
--  1-1) 연결할 두 테이블의 컬럼명이 다른 경우
--  EMPLOYEE 테이블과 DEPARTMENT 테이블을 조인하여 사번, 사원명, 부서 코드, 부서명을 조회 (EQUAL JOIN)
--  일치하는 값이 없는 경우 조회에서 제외된다. (DEPT_CODE가 NULL인 사원, DEPT_ID D3, D4, D7인 사원)
...
-- ANSI 구문
--  2-1) 연결할 두 테이블의 컬럼명이 다른 경우
--  EMPLOYEE 테이블과 DEPARTMENT 테이블을 조인하여 사번, 사원명, 부서 코드, 부서명을 조회 (INNER JOIN)
SELECT E.EMP_ID, E.EMP_NAME, E.DEPT_CODE, D.DEPT_TITLE
FROM EMPLOYEE E
/*INNER*/ JOIN DEPARTMENT D ON E.DEPT_CODE = D.DEPT_ID;
--/*INNER*/ JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID); -- 괄호로 묶어줘도 된다.
...
FROM EMPLOYEE E
JOIN JOB J USING(JOB_CODE);   -- USING은 같은 컬럼이라고 인식해서 ambiguously가 발생하지 않는다.
...
--  방법 3) NATURAL JOIN을 이용하는 방법 (참고용)
--  JOIN을 기술해주지 않아도 두 개의 테이블에서 동일한 컬럼을 알아서 조인해준다.
--  동일한 컬럼은 무조건 조인시켜주기 때문에 원하는 결과가 나오지 않을 수도 있다.
--  동일한 이름의 컬럼을 자동으로 조인해줄 때 USING과 동일하게 테이블, 별칭명을 생략해준다.
SELECT E.EMP_ID, E.EMP_NAME, JOB_CODE, J.JOB_NAME
FROM EMPLOYEE E
NATURAL JOIN JOB J;
...
-- FROM EMPLOYEE
-- NATURAL JOIN JOB;     -- 조인하고자 하는 컬럼을 확인하기 어렵다.
...
-- 오라클 구문
SELECT D.DEPT_ID, D.DEPT_TITLE, L.LOCAL_CODE, D.LOCATION_ID
...
WHERE DEPT_ID = DEPT_CODE AND NVL(E.BONUS, 0) > 0 ;
...
-- 4. EMPLOYEE 테이블과 DEPARTMENT 테이블, LOCATION 테이블을 조인해서 사번, 사원명, 부서명, 지역명 조회
-- 오라클 구문
...
-- 6. 사번, 사원명, 부서명, 지역명, 국가명, 급여 등급 조회(NON EQUAL JOIN후에 실습 진행)
-- 오라클 구문
SELECT E.EMP_ID,
        E.EMP_NAME,
        D.DEPT_TITLE,
...
/*
        2. 외부 조인(OUTER JOIN)
            테이블 간에 JOIN 시 일치하지 않는 행도 포함시켜서 조회가 가능하다.
...
-- ANSI 구문
-- 부서 코드가 없던 (이오리, 하동운)의 정보가 나오게 된다.
SELECT E.EMP_NAME, D.DEPT_TITLE, E.SALARY, E.SALARY * 12
FROM EMPLOYEE E
...
-- 오라클 구문 (오른쪽 조건 컬럼에 해당하는 데이터 JOIN 진행)
SELECT E.EMP_NAME, D.DEPT_TITLE, E.SALARY, E.SALARY * 12
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_CODE(+) = D.DEPT_ID;
...
-- 오라클 구문 (에러 발생 - OUTER JOIN을 하나밖에 지원을 해주지 않아서 한 쪽에만 표시 가능)
SELECT E.EMP_NAME, D.DEPT_TITLE, E.SALARY, E.SALARY * 12
FROM EMPLOYEE E, DEPARTMENT D
WHERE E.DEPT_CODE(+) = D.DEPT_ID(+);
...
ORDER BY EMP_NAME;  -- 23 * 9의 결과 행 출력 -> 207
...
-- EMPLOYEE 테이블을 SELF JOIN 하여 사원번호, 사원 이름, 부서 코드, 사수 번호, 사수 이름 조회
-- ANSI 구문
...
-- WHERE N.NATIONAL_CODE = '한국' OR N.NATIONAL_CODE = '일본';
