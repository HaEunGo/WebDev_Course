    ...
    <INSERT>
        ...
        [표현법]
            ...
            2) INSERT INTO 테이블명(컬럼명, 컬럼명, ..., 컬럼명) VALUES(값, 값, ..., 값);
                ...
                선택이 안된 컬럼들은 기본적으로 NULL 값(기본값)이 들어간다. (NOT NULL 제약조건이 걸려있는 컬럼은 반드시 선택해서 값을 제시해야 한다.)
                단, 기본값(DEFAULT)이 지정되어 있으면 NULL이 아닌 기본값이 들어갔다.
...
VALUES ('900', '공유', '901008-1000302', 'kong@kh.or.kr', '01012341234', 'D1', 'J7',
        4300000, 0.2, '200', SYSDATE, NULL, DEFAULT);

-- 표현법 2)번 사용
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME, EMP_NO, JOB_CODE)
VALUES('901', '인절미', '991008-1111999', 'J7');   -- (901)로 적어도 자동 형변환되지만 웬만하면 데이터 타입을 맞춰서 넘겨주는 게 좋다.
...
-- WHERE 조건절을 주지 않으면 모든 행들의 이름(DEPT_TITLE)이 변경된다.
UPDATE DEPT_COPY
...
-- 선동일 사원의 급여를 700만원으로, 보너스를 0.2로 변경하시오
UPDATE EMP_SALARY
SET SALARY = 7000000,
    BONUS = 0.2
WHERE EMP_NAME = '선동일';

-- 모든 사원의 급여를 기존 급여에서 10% 인상한 금액 (기존 급여 * 1.1)으로 변경
UPDATE EMP_SALARY
...
UPDATE EMP_SALARY
SET EMP_ID = NULL
WHERE EMP_ID = 200;     -- NOT NULL 제약조건에 위배된다.

-- EMPLOYEE 테이블에서 노옹철 사원의 부서 코드를 'D0'으로 변경
UPDATE EMPLOYEE
SET DEPT_CODE = 'D0'    -- FOREIGN KEY 제약 조건에 위배된다.
WHERE EMP_NAME = '노옹철';
...
-- 방명수 사원의 급여와 보너스들을 유재석 사원과 동일하게 변경
SELECT SALARY, BONUS
FROM EMP_SALARY
WHERE EMP_NAME = '유재식';
...
SET SALARY = (
    ...
    )                         -- 이대로 실행하면 모든 사원의 SALARY, BONUS가 변경됨
WHERE EMP_NAME = '방명수';     -- WHERE절에 조건식을 넣어 '방명수'사원만 변경될 수 있도록 한다.
...
-- 노옹철, 전형돈, 정중하, 사원들의 급여와 보너스를 유재식 사원과 동일하게 변경
UPDATE EMP_SALARY
SET (SALARY, BONUS) = (
    SELECT SALARY, BONUS
    FROM EMP_SALARY
...
-- 1. 아시아 지역에서 근무하는 직원들 조회 (내가 해본 방법)
SELECT SALARY, BONUS
FROM EMPLOYEE E
...
-- 2. 아시아 지역에서 근무하는 직원의 보너스를 0.3으로 변경 (서브쿼리)
UPDATE EMP_SALARY
SET (SALARY, BONUS) = (
    SELECT E.SALARY, E.BONUS
    FROM EMPLOYEE E
    JOIN DEPARTMENT D ON(E.DEPT_CODE = D.DEPT_ID)
    JOIN LOCATION L ON(D.LOCATION_ID = L.LOCAL_CODE)
    WHERE BONUS = 0.3
)
WHERE L.LOCAL_NAME LIKE 'ASIA%'; -- "LOCAL_NAME": invalid identifier, "%s: invalid identifier"
...
ROLLBACK;   -- 마지막 커밋 시점으로 ROLLBACK된다.
...
-- SELECT * -- 삭제할 데이터 미리 확인 후 삭제하는 것이 좋음
DELETE FROM DEPARTMENT
WHERE DEPT_ID = 'D1';   -- D1의 값을 참조하는 자식테이블에 데이터가 있기 때문에 삭제가 되지 않는다. 참조 관계에 있는 행은 삭제가 불가능 (자식테이블에서 사용중)

-- DEPARTMENT 테이블에서 DEPT_ID가 D3인 부서를 삭제
DELETE FROM DEPARTMENT
WHERE DEPT_ID = 'D3';   -- D3의 값을 참조하는 자식 테이블 데이터가 없기 때문에 삭제가 된다.(제약조건) 참조 관계에 없는 행은 삭제 가능 (자식테이블에서 사용중이기 때문)
