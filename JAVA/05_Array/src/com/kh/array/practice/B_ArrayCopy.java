...
public class B_ArrayCopy {
	 ...
	 *  		Arrays.copyOf()    : 복사하고자 하는 배열의 크기만 다시 지정해서 복사하고 싶을 때 사용한다.
	...
	public void method1() {
		 ...
		 *   - 수행문장을 실행하고 다시 배열에서 가져올 값이 존재한다면 해당 값을 변수에 저장 후 수행문자를 실행하고 다음 값이 없으면 for 문을 종료한다.
		 *  
		 */
		...
		copy [3] = 44; 	// copy 배열의 값 목록 => 1, 2, 3, 44, 5 -> 실제 Heap영역에 있는 값이 바뀌었기 때문에 이후 배열 출력 시 변경된 값으로 출력된다. (같은 주소값을 호출하기 때문에 원본, 복사본이 동일하게 뜬다.)
	...
	public void method2() {
		...
		//주소값확인 (hashcode) -> 값 대입 후 각각 달라진 주소값 확인 가능하다.
		System.out.println("\norigin의 주소 값 : " + origin.hashCode());
		System.out.println("copy의 주소 값 : " + copy.hashCode());
	}
	...
	public void method3() {
		// 2. System 에서 제공하는 arraycopy() 메소드를 이용한 깊은 복사
		int[] origin = {1, 2, 3, 4, 5};
		int[] copy = new int[10];		// copy배열 미리 생성
		
		// System.arraycopy(원본 배열명, 복사할 시작 인덱스, 복사본 배열명, 복사본 배열의 복사될 시작 위치, 복사할 길이);
	...
	public void method4() {
		...
		// method3과 다른 점 : Arrays 클래스는 배열을 이용할 때 유용한 메소드들을 제공한다.(직접 생성X)
	...
	public void method5() {
		...
		// 원본 배열을 통째로 복사해서 새로운 배열을 생성 (시작 인덱스 지정X, 복사본 배열의 크기 지정 X)
		int[] origin = {1, 2, 3, 4, 5};
