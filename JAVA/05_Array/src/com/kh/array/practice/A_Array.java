...
public class A_Array {
	 ...
	 * 
	 * 		4-2) 선언과 동시에 초기화
	 * 				자료형[] 배열명 = {값1, 값2, 값3, ...};	 // 값의 목록을 나열하면 그걸 보고 JVM이 값의 목록을 담을 수 있는 배열을 실제 Heap영역에 담아주고, 그 주소값을 배열명에 전달해준다.
	 * 				자료형 배열명[] = {값1, 값2, 값3, ...};
	 * 				자료형[] 배열명 = new 자료형[] {값1, 값2, 값3, ...};
	 * 				자료형 배열명[] = new 자료형[] {값1, 값2, 값3, ...};
	 * 			
	 * 
	 * 		4-3) 반복문을 이용한 초기화 (for, while)
	 * 			
	 */
	...
	public void method1() {
		...
		System.out.println(num7);		// 비효율적이고 불편하다.
	...
	public void method2() {
		// 변수 선언
		int num;				// 일반 변수 : 메모리(Stack)영역에 자료형 만큼의 크기로 값을 담을 수 있는 공간 할당된다.
		...
		// 배열 할당
//		iArray1 = new int[];		// 배열의 크기를 지정하지 않으면 에러가 발생한다. -> 배열의 index는 0부터 시작해야한다.
		...
		// 배열을 생성하고 초기화해주지 않아도 문제없이 값이 출력이 된다.
		// 배열은 따로 초기화 하지 않는다면, JVM이 지정한 기본 값으로 배열이 초기화된다.
		// (정수형 : 0, 실수형 : 0.0, 문자형 : '\u0000', 논리형 : false, 참조형(문자열) : null)
		for(int i = 0; i< iArray1.length; i++) {
			System.out.printf("iArray1[%d] : %d\n]", i, iArray1[i]);
		}
		
		// 배열명.length를 통해서 배열의 길이(크기)를 알 수 있다. 
		for(int i = 0; i< dArray1.length; i++) {
			System.out.printf("dArray1[%d] : %f\n]", i, dArray1[i]);
		}
		
		// 값을 초기화(저장)하는 for 문 -> 배열에 값을 인덱스에 저장하는 것.
		for(int i = 0; i < iArray2.length; i++) {
			iArray2[i] = (i + 1) * 2;
//			iArray2[i] = i * 2;
		}
		
		// 값을 출력하는 for 문 -> 배열의 값을 인덱스에서 읽어오는 것.
		for(int i = 0; i < iArray2.length; i++) {
	...
	public void method3() {
		int iArray[] = new int [5];		// 선언과 동시에 초기화
		...
		iArray[3] = 8;
		iArray[4] = 10;
//		iArray[5] = 12;	// 지정한 배열의 크기보다 더 큰 인덱스의 값을 지정하려고 하는 경우 -> ArrayIndexOutOfBoundsException이 발생한다.
		...
		// hashCode() : 주소값을 10진수 값으로 반환한다. (자바 객체에서 해시 코드는 객체를 식별하는 하나의 정수값을 말한다.)
		System.out.println(iArray.hashCode());
		
		// 배열에서 한 번 지정한 크기의 변경은 불가능하다.
		// 따라서 배열의 크기를 변경하려면 다시 배열 크기를 지정해서 새로 생성해야 한다.
		iArray = new int[6];
		
		
		// 기존에 참조하고 있던 배열과의 연결이 끊기고 새로운 배열을 참조하게 된다.
		// 기존에 참조하고 있던 배열은 더 이상 참조하는 변수가 없기 때문에 가비지 컬렉터의 대상이 된다.
		// (특정한 시점에 가비지 컬렉터에 의해서 정리된다.)
		System.out.println(iArray.length);
		System.out.println(iArray.hashCode());
		
		// 참조 타입 변수는 Heap영역의 배열(객체)를 참조하지 않는다는 뜻으로 null(널)값을 가질 수 있다.
		// 참조 변수에 null 값을 넣어주면 참조하고 있는 주소가 없어지는데 이때 heap에 존재하는 배열은 더 이상 참조되지 않는 배열이 된다. (연결이 끊기는 것)
		// 더 이상 사용이 되지 않는 배열은 가비지 컬렉터에 의해 일정 시간이 지난 후에 삭제된다.
		iArray = null;
		
		// 개발자가 null 값을 가지고 있는 참조 변수를 사용하면 NullPointerException이 발생한다. -> 아래 코드를 그대로 실행하면 에러발생
		System.out.println(iArray.length);
		System.out.println(iArray.hashCode());
...
//		int[] iArray2;
//		iArray2 = {6, 7, 8, 9, 10};		// 반드시 선언과 동시에 초기화를 진행해주어야 한다.
//		iArray2 = new int[] {6, 7, 8, 9, 10};	
		...
		// 0번 인덱스부터 마지막(배열의 크기보다 작을 때 : 2 )까지 반복으로 사용자에게 문자열을 입력받아 해당 인덱스에 사용자가 입력한 값 넣기 
