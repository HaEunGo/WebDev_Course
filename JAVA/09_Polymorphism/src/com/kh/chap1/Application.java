...
/*
 * 다형성
 * 		- 부모 타입으로부터 파생된 여러 가지 자식 객체를 부모 클래스의 참조 변수로 다룰 수 있는 기술이다.
 ...
 * 		- 형변환 연산자 생략이 불가능하다. (명시적 형변환 필요)
 * 		- ((자식타입)부모참조변수).자식의멤버(메소드)();
 * 
 */
...
public class Application {
	...
	public static void main(String[] args) {
		...
		// product 참조변수로 Product의 멤버만 접근이 가능하다.
		System.out.println(product.information());
		
		// 2. 자식 타입의 레퍼런스로 자식 객체를 다루는 경우
		Desktop desktop = new Desktop();
		
		// desktop 참조변수로 Product, Desktop의 멤버에 접근이 가능하다.
		System.out.println(desktop.isAllInOne());
		System.out.println(desktop.information());
		
		// 3. 부모 타입의 레퍼런스로 자식 객체를 다루는 경우 (다형성 적용) (UpCasting)
		Product product2 = /* (Product) */new Desktop();		// 상속구조로 연결
		
		// product2 참조변수로 Product의 멤버만 접근이 가능하다.
		// 하지만 Desktop의 멤버에 접근하고 싶으면 DownCasting(강제(명시적)형변환)을 해줘야 한다.
		...
		for (int i =0; i < arr3.length; i++) {
			...
			if(arr3[i] instanceof Desktop) {
				...
				System.out.println("isAllInOne : " + ((SmartPhone)arr3[i]).getMobileAgency());

			}
		}
		
		Product product3 = new Desktop("애플", "IMac-1", "Mac", 1300000, false);
		Product product4 = new SmartPhone("애플", "A-01", "아이폰 SE", 200000, "SK");
		Product product5 = new Tv("LG", "T-1", "스마트TV", 2000000, 60);
		
		/*
		 *  다형성에 오버라이딩의 개념을 접목하게 되면 실제 참조하고 있는 클래스의 오버라이딩된 메소드를 찾아가서 실행한다.
		 *	동적 바인딩 : 프로그램이 실행되기 전에 컴파일 되면서 모든 메소드는 정적 바인딩이 되는데, 
