...
public class A_BufferedStream {
	 ...
	 * 		- 버퍼라는 공간을 사용해서 속도 향상에 도움을 준다. (버퍼라는 공간에 데이터를 쌓아놨다가 한 번에 입/출력 시킨다.)
	...
	public void fileSave() {
		...
		try (BufferedWriter bw = new BufferedWriter(new FileWriter("c_buffer.txt", true))) {
			...
			bw.write("안녕하세요~\n");
			bw.write("오늘은 YYYY년 MM월 DD일 입니다.");
			bw.newLine();		//기반 스트림에서 제공하지 않고, 보조 스트림에서 제공하는 메소드, 개행할 때 사용.)
			bw.write("명탐정 코난");
			bw.newLine();
			bw.write("동해물과백두산이");
			bw.flush();
	...
	public void fileRead() {
		...
		try(BufferedReader br = new BufferedReader(new FileReader("c_buffer.txt"))) {
			...
			// 기반 스트림에서는 제공하지 않는 메소드, 파일에서 한 줄 읽어올 때 사용한다.
