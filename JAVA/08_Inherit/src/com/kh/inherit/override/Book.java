...
public class Book /* extends Object */{
	 ...
	 *			(부모 클래스에서 정의된 메소드가 사라지는 것이 아니다.)
	 *
	 *	오버라이딩 성립 조건
	 *		- 부모 클래스의 메소드와 메소드명이 동일해야한다.
	 *		- 매개변수의 개수, 자료형, 순서가 동일해야한다.
	 *		- 반환형이 동일해야한다.
	 ...
	 *  - 기본적으로 Object 클래스에서는 "클래스명@16진수 해시코드" 값을 리턴하는데 자식 클래서에서 객체에 대한 유용한 문자열 정보를 리턴하도록 재정의할 수 있다.
	 *  
	 *  - print 메소드의 경우 기본적으로 객체의 참조 값을 매개값으로 넘겨주면 내부적으로 참조값.toString() 메소드를 호출해서 리턴되는 값을 출력해준다.
	 */
	

//	@Override			//override / implement method를 이용한 생성
...
//	
	...
	@Override			// generate toString을 이용한 생성
	public String toString() {
	 ...
	 *  	- 객체의 동등 비교 시 주소값 비교가 아닌 객체가 가지고 있는 필드의 값들이 모두 같을 경우 true가 반환되고, 하나라도 다른 경우면 false가 반환되도록 재정의해서 사용할 수 있다.
	 *  
	 *  
	 */
...
//	}
	...
	public boolean equals(Object obj) {
		...
		Book target = (Book) obj;	// 형변환
		
		// private은 해당 클래스에서 직접 접근이 가능하기 때문에 아래와 같이 target의 private멤버에도 직접 접근이 가능하다.
		if (this.title.equals(target.getTitle()) && this.author.equals(target.getAuthor()) && this.price == target.price) {
			result = true;
		}	
	 ...
	 *  	- 기본적으로 Object 클래스의 hashCode() 메소드는 객체의 메모리 번지(주소)를 이용해서 만들어주기 때문에 객체마다 다른 값을 가지고 있다.
	...
	public int hashCode() {
		 ...
		 * 		- title + author + price ---> 결과값은 문자열이 된다.
		 *  String 클래스에서 같은 문자열일 경우 해당 문자열의 hashCode는 같도록 이미 재정의 되어있다.
		 */
		return (this.title + this.author + this.price).hashCode();
		// = "자바의 정석남궁성3000".hashCode();
	}
