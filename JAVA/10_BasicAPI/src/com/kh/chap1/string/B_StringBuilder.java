...
public class B_StringBuilder {
	...
	public void method2() {
		...
		// 기존 문자열 뒤에 문자열 추가하기
		// 버퍼의 크기가 부족하면 크기가 늘어나되 줄어들진 않는다.
//		str = str + "저는 홍길동입니다.";
		str += "저는 홍길동입니다.";
		sb.append("저는 홍길동입니다.");
		...
		System.out.println("sb의 버퍼의 크기 : " + sb.capacity());
		System.out.println(System.identityHashCode(str));
		System.out.println(System.identityHashCode(sb));		// 주소값 그대로
		System.out.println();
		...
		// 주소의 값은 그대로, 2번째부터 5번째까지의 문자열 삭제한 뒤 출력된다.
		sb.delete(2, 5);
		...
		// reverse() : 문자열의 순서를 역으로 바꿔서 출력해준다.
		sb.reverse();
		...
		// why? 우리가 사용했던 아래의 메소드들은 반환형이 StringBuilder이고, 실제 반환되는 결과값이 해당 객체의 주소값이 반환되기 때문에 메소드 체이닝이 가능하다.(return this : 자기자신의 참조값을 리턴)
