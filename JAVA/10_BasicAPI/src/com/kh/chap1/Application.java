...
public class Application {
	 ...
	 * 		- 변경이 적고, 읽기가 많은 경우에 사용하는 것이 좋다. (여러 개의 연산을 할 때마다 메모리공간을 새로 생성함)
	 * 	
	 * 	3. StringBuilder & StringBuffer
	 * 		- StringBuilder, StringBuffer 클래스로 생성된 객체는 가변 객체이다.
	 * 		- 16개의 문자를 저장할 수 있는 버퍼가 미리 생성되고 문자가 저장됨에 따라서 자동으로 증가한다.
	 * 		- StringBuilder와 StringBuffer의 차이점은 동기화가 되느냐 안 되느냐의 차이다.
	 * 			- 멀티 스레드 환경 -> StringBuffer 사용 권장
	 * 			- 단일 스레드 환경 -> StringBuilder 사용 권장
	 * 
	 * 	4. StringTokenizer
	 * 		- 생성자로 전달받은 문자열을 구분자를 이용하여 분리하는 클래스이다.
	 * 		- 분리된 최소 단위(최소한의 문자열)를 토큰이라고 부른다.
	 * 
	 */
